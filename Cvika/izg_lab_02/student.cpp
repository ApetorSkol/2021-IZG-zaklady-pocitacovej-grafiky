...
void drawLine(int x1, int y1, int x2, int y2, S_RGBA color)
{
	...
	int k ; // koeficient linearnej funkcie

	if (dx == 0) // ak sa jedna o bod alebo o priamku kolmu na x-ovu os je k=0
	{
		k = 0;
	}
	else
	...
	if (abs(dy) > abs(dx)) // ak mame vykreslit priamku pre "y"
	{
		// vymena suradnic
		int tmp = x1;
		x1 = y1;
		...
		k = (dx << FRAC_BITS) / dy; // koeficient je urceny tak aby ratal kolko x pripada na y
	}

	if (x1 > x2) // ak kreslime od vacsieho k mensiemu
	{
		int tmp = x1;
		x1 = x2;
		x2 = tmp;
	...
	auto first_point{ y1 << FRAC_BITS }; // prvy bod ktorym zacina zakreslenie priamky
    ...
    for (int x = x1; x <= x2; ++x)
    {
		...
		if (abs(dy) > abs(dx))
		{
			setPixel(first_point >> FRAC_BITS , x , color); // po prevedeni vymeny "x" nadobuda hodnoty y
		}
		else
		{
			setPixel(x, first_point >> FRAC_BITS, color);
		}
		first_point += k;
    }
}
