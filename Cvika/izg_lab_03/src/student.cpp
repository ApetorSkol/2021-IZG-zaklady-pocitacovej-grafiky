...
void pinedaTriangle(const Point &v1, const Point &v2, const Point &v3, const RGBA &color1, const RGBA &color2, bool arrow)
{
    ...
    //////// DOPLNTE KOD /////////
	Point min = v1;
	min.x = min.x > v2.x ? v2.x : min.x;
	min.x = min.x > v3.x ? v3.x : min.x;
	min.y = min.y > v2.y ? v2.y : min.y;
	...
	Point borders[3] =
	{
		...
		Point(v3.x - v2.x , v3.y - v2.y),
		Point(v1.x - v3.x , v1.y - v3.y)
	};
    ...
    //////// DOPLNTE KOD /////////
	int E_start[3];
	E_start[0] = (min.y - v1.y)*borders[0].x - (min.x - v1.x)*borders[0].y;
	E_start[1] = (min.y - v2.y)*borders[1].x - (min.x - v2.x)*borders[1].y;
	E_start[2] = (min.y - v3.y)*borders[2].x - (min.x - v3.x)*borders[2].y;

	int E_row_start[3] = { E_start[0], E_start[1], E_start[2] };

	for (int x = min.x; x <= max.x; x++) {
		int startY = (x - min.x) % 2 ? max.y : min.y;
		int endY = (x - min.x) % 2 ? min.y - 1 : max.y + 1;
		int direction = (x - min.x) % 2 ? -1 : 1;
		for (int y = startY; y != endY; y += direction) {
			bool inside = true;
			for (int i = 0; i < 3; i++)
				inside = inside && (E_row_start[i] >= 0);
			if (inside)
				putPixel(x, y, color1);
			for (int i = 0; i < 3; i++)
				E_row_start[i] += (x - min.x) % 2 ? -borders[i].x : borders[i].x;
		}
		for (int i = 0; i < 3; i++)
			E_row_start[i] -= borders[i].y;
	}
...
void pinedaPolygon(const Point *points, const int size, const RGBA &color1, const RGBA &color2)
{
	...
	if (size < 1)
		IZG_ERROR("Mnohouholnik s 0 bodmi.\n");
	...
	{
		edge_params[i].deltaX = points[(i + 1) % size].x - points[i].x;
		edge_params[i].deltaY = points[(i + 1) % size].y - points[i].y;
		edgeFncValues[i] = (min.y - points[i].y)*edge_params[i].deltaX - (min.x - points[i].x)*edge_params[i].deltaY;
	}
	...
	// hrany mame zapisane ako vektory
	// vyratame si vektorovy sucin dvoch susednych hran
	// ak je vektorovy sucin je zaporny tak utvar je konkavny
	// v1 = ( x1 , y1 , 0  )
	// v2 = ( x2 , y2 , 0  )
	// parameter z je 0 lebo sa jedno o 2D objekt
	// sucin v1xv2 = y1   0   x1    y1
	//                  x   x    x
	//               y2   0   x2    y2
	// v1xv2 = y1*0 - y2*0 + 0*x2 - 0*x1 + x1*y2 - x2*y1
	// v1xv2 = x1*y2 - x2*y1
	for (int i = 0; i < size; i++)
	{
		int vect_mul = (edge_params[i].deltaX*edge_params[(i + 1) % size].deltaY - edge_params[(i+1)%size].deltaX*edge_params[i].deltaY);
		if (vect_mul < 0)
		{
			IZG_ERROR("Mnohouholnik nieje konvexny.\n");
		}
	}
	...
	for (int x = min.x; x <= max.x; x++) {
		...
		for (int y = startY; y != endY; y += direction) {
			bool inside = true;
			for (int i = 0; i < size; i++)
				inside = inside && (edgeFncValues[i] >= 0);
			if (inside)
				putPixel(x, y, color1);
			for (int i = 0; i < size; i++)
				edgeFncValues[i] += (x - min.x) % 2 ? -edge_params[i].deltaX : edge_params[i].deltaX;
		}
		for (int i = 0; i < size; i++)
			edgeFncValues[i] -= edge_params[i].deltaY;
	}
