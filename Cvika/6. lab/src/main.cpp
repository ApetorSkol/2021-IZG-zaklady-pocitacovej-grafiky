...
std::string loadTxtFile(std::string const&fileName){
  ...
  if(!file.is_open()){
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR,("cannot open file: "+fileName).c_str(),("text file: "+fileName+" cannot be opened").c_str(),nullptr);
    return "";
  }
...
GLuint createShader(GLuint type,std::string const&src){
  ...
  if(compileStatus != GL_TRUE){
    ...
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR,(shaderTypeToName(type)+" shader compilation error").c_str(),msg,nullptr);
  }
  return vs;
}
...
GLuint createProgram(GLuint vs,GLuint fs){
  ...
  if(linkStatus != GL_TRUE){
    ...
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR,"program linking error",msg,nullptr);
  }
  return prg;
}
...
Frame loadFrame(std::string const&file){
  ...
  if(!data){
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR,"loadFrame",("cannot load image: "+file).c_str(),nullptr);
    return res;
  }
...
int main(int argc,char*argv[]){
  ...
  auto triangles           = args->getu32   ("--triangles"       ,100000                                             ,"number of triangles"    );
  ...
  auto vs  = createShader(GL_VERTEX_SHADER,"#version 330\n#define VERTEX_SHADER\n"+src);
  auto fs  = createShader(GL_FRAGMENT_SHADER,"#version 330\n#define FRAGMENT_SHADER\n"+src);
  ...
  while(running){
    ...
    glProgramUniformMatrix4fv(prg,viewL,1,GL_FALSE,glm::value_ptr(viewMatrix));
    glProgramUniformMatrix4fv(prg,projL,1,GL_FALSE,glm::value_ptr(projMatrix));
    glProgramUniform1f(prg,timeL,timer.elapsedFromStart());

    int w,h;
    SDL_GetWindowSize(window,&w,&h);
    glProgramUniform2f(prg,resL,w,h);
